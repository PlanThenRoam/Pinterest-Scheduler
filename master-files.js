...
(() => {
 ...
 function publicationLabel(r){
  ...
  const delivered=r.files.filter(f=>['docx','pdf','zip'].includes(f.name.split('.').pop().toLowerCase()));
 ...
 async function startEtsy(){
  ...
  try{
   ...
   $('#masterEtsyStage').innerHTML=`<form id="masterEtsyForm">${detail.master.files.map((f,i)=>`<label>${esc(f.name)}<select name="file_${i}"><option value="">Do Not Include</option><option value="add">Add As New Download</option>${existing.map(x=>`<option value="${esc(x.id)}">Replace ${esc(x.name)}</option>`).join('')}</select></label>`).join('')}<p class="muted">${existing.length}/5 current Etsy files. Replacement requires a free slot while the new upload is confirmed.</p><button class="btn primary" type="submit">Save Files For Etsy Review</button></form>`;$('#masterEtsyForm').onsubmit=stage;
