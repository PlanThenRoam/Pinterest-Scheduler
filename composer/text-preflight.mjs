...
export async function measureText(page){
 return page.evaluate(()=>[...document.querySelectorAll('.copy')].map(el=>{
  ...
  function inspect(){
   ...
   for(const m of text.matchAll(/\S+/g)){const range=document.createRange();range.setStart(span.firstChild,m.index);range.setEnd(span.firstChild,m.index+m[0].length);const rect=range.getBoundingClientRect(),paragraph=text.slice(0,m.index).split('\n').length-1;let line=lines.find(l=>l.paragraph===paragraph&&Math.abs(l.y-rect.y)<1);if(!line){line={paragraph,y:rect.y,words:[],width:0,left:rect.x};lines.push(line);}line.words.push(m[0]);line.width=rect.right-line.left;}
   ...
   return {name:el.dataset.layer,text,font_size:parseFloat(style.fontSize),font_family:style.fontFamily,x:r.x,y:r.y,width:r.width,height:r.height,lines:lines.map(l=>({text:l.words.join(' '),width:Math.round(l.width*100)/100})),isolated_words:orphans,overflow:s.height<=0||s.width<=0||span.scrollWidth>width+0.5||s.height>height+0.5,wrap_mode:span.style.textWrap||'wrap'};
  }
