...
Portrait backgrounds must be verified 1000 × 1500 assets. Following the owner’s explicit cropping instruction, `portrait-assets.mjs` creates these from the 190 locked 1080-square sources. Its default crop is the central 720 × 1080 rectangle, resized proportionally to 1000 × 1500. A private per-source crop file can shift the frame when required. The square sources are never overwritten. Ingest the resulting portrait PNGs through the existing private asset ingestion procedure, with separate logical keys, exact source and output checksums, and the recorded crop rectangle. The composition renderer requires the prepared portrait asset and does not stretch a square image. Genuine current DOCX pages are shared by both formats.
...
Deployment alone does not supply portrait artwork. An empty portrait bank is reported as `NEEDS_BACKGROUNDS` by the planner brief. Before production, complete and inspect the portrait bank; never treat an unconverted square asset as compatible. Run `node composer/portrait-assets.mjs PRIVATE_SOURCE_MANIFEST PRIVATE_OUTPUT_DIRECTORY [PRIVATE_CROP_OVERRIDES]` to generate the individual PNGs and private inspection sheets. The script verifies all 190 sources, their checksums and the ten-per-planner distribution; failures remain explicit in the private output manifest. Neither manifests, source bytes, inspection sheets nor crop outputs belong in this public repository.
