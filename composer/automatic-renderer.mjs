...
export async function renderAutomatic(session,spec,assets,loadBytes,{preflightOnly=false,scope,fontBank,sha}){
 ...
 const panels=d.style_scope==='campaign'?'<div data-campaign-fade style="position:absolute;inset:0;z-index:1;background:linear-gradient(180deg,rgba(0,0,0,.64),rgba(0,0,0,.58) 42%,rgba(0,0,0,.60) 72%,rgba(0,0,0,.68))"></div>':layout.panels.map(l=>`<div data-backdrop="${l.name}" style="${position(l)}${l.outline?`border:2px solid ${d.palette.ink};`:''}${l.accent?`border-left:5px solid ${l.accent};`:''}border-radius:${l.radius||0}px;background:${l.gradient?`linear-gradient(90deg,${rgba(l.colour,l.opacity)},${rgba(l.colour,l.opacity*.96)} 85%,${rgba(l.colour,l.opacity*.84)})`:rgba(l.colour,l.opacity)}"></div>`).join('');
 ...
 try{
  ...
  if(!preflight.valid||!preflight.exact_text){const e=Error('COPY_DOES_NOT_FIT: '+measured.filter(x=>x.overflow||x.isolated_words.length).map(x=>x.name).join(', ')+'; choose a larger text layout or supply approved line breaks. Campaign font sizes stay fixed. No words were removed.');e.preflight=preflight;e.timings={...timings,total_ms:elapsed(started)};throw e;}
  ...
  const resolved={...layout,...(d.style_scope==='campaign'?{text_effect:{back_panels:false,shadow:'0 2px 6px rgba(0,0,0,.35)',image_fade:[.64,.58,.60,.68]}}:{}),layers:layout.layers.map(l=>({...l,...measured.find(x=>x.name===l.name)}))};
  ...
  for(const item of localPanels){if(d.colour_mode==='fixed'){contrastChoices.push({name:item.name,unboxed:false,colour:d.blocks[item.name].colour});continue;}const candidates=[d.blocks[item.name].colour,d.palette.panel].map(colour=>{const rgb=colour.slice(1).match(/../g).map(x=>parseInt(x,16)),ink=luminance(rgb),ratios=[];for(const r of item.rects)for(let y=Math.max(0,Math.ceil(r.y));y<Math.min(layout.height,Math.floor(r.y+r.height));y+=4)for(let x=Math.max(0,Math.ceil(r.x));x<Math.min(layout.width,Math.floor(r.x+r.width));x+=4){const i=(y*layout.width+x)*3,bg=luminance([...photoPixels.subarray(i,i+3)]);ratios.push((Math.max(ink,bg)+.05)/(Math.min(ink,bg)+.05));}ratios.sort((a,b)=>a-b);return {colour,ratio:ratios[Math.floor(ratios.length*.08)]||0};});const floor=item.name==='headline'?3.3:5,chosen=candidates.find(c=>c.ratio>=floor);contrastChoices.push({name:item.name,unboxed:!!chosen&&!item.hasEmphasis&&d.colour_mode!=='fixed',colour:chosen?.colour||d.blocks[item.name].colour});}
