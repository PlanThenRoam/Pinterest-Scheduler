...
async function processJob(job){
 ...
 try{
  ...
  let tick=Date.now();const {upload_url}=await call('prepare_result',args);const uploaded=await fetch(upload_url,{method:'PUT',headers:{'Content-Type':'image/png'},body:r.png,signal:AbortSignal.timeout(120000)});if(!uploaded.ok)throw Error('PNG upload failed');r.timings.result_upload_ms=Date.now()-tick;stats.result_upload_ms+=r.timings.result_upload_ms;
