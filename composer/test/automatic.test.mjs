...
import {resolveAutomatic,compareDesign,acceptableDesign,automaticLayout,LAYOUT_FAMILIES,TREATMENTS,CTA_TREATMENTS,SYSTEMS,sourceIdentity,fontSupported} from '../../supabase/functions/composer/art-direction.mjs';
...
test('two campaigns are structurally different, each carousel coherent, proof arrangements varied and pins distinct',()=>{
 ...
 for(const r of [one,two]){assert.equal(r.outputs.length,7);assert.equal(new Set(r.outputs.slice(0,5).map(o=>JSON.stringify(o.composition.design.fonts))).size,1);assert.ok(new Set(r.outputs.slice(1,4).map(o=>o.composition.design.layout_family)).size>=3);assert.ok(compareDesign(...r.signatures.pins).major_differences>=3);assert.ok(compareDesign(...r.signatures.pins).score<.69);assert.equal(r.outputs[0].composition.planner_page_ids.length,0);assert.equal(r.outputs[4].composition.planner_page_ids[0],assets.find(a=>a.kind==='page').id);assert.ok(r.outputs.slice(0,4).every(o=>!o.composition.cta));}
 assert.ok(compareDesign(one.signatures.carousel,two.signatures.carousel).major_differences>=3);assert.ok(two.exceptions.length>=4);assert.ok(two.exceptions.every(x=>x.code==='BACKGROUND_LRU_REUSE'));
...
test('all twelve headline treatments and nine CTA treatments pass actual measured rendering',async()=>{
 ...
 const wanted=new Map([...TREATMENTS.map(t=>['type:'+t,null]),...CTA_TREATMENTS.map(t=>['cta:'+t,null])]);
 ...
 assert.ok([...wanted.values()].every(Boolean),'Every preset must be reachable from automatic generation');
