...
test('all twenty approved font families load and visibly render exact copy',async()=>{for(const font_family of FONTS){const result=await render({...base,font_family},[bg],async()=>bytes);assert.equal(result.validation.font_loaded,true);assert.equal(result.validation.font_families,1);assert.ok(result.validation.visible_text.every(x=>x.visible_pixels>20));}});
test('re-render has identical PNG bytes and preserves explicit line breaks',async()=>{const a=await render(base,[bg],async()=>bytes),b=await render(base,[bg],async()=>bytes);assert.equal(a.checksum,b.checksum);assert.equal(a.layout.layers.find(l=>l.name==='headline').text,base.headline);});
test('long copy, insufficient contrast, unsupported glyphs and corrupted assets fail closed',async()=>{
