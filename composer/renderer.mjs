...
export async function render(spec,assets,loadBytes){
 ...
 const browser=await playwright.launch({headless:true,executablePath:await browserPath(),args:chromium.args});
 try{
  const page=await browser.newPage({viewport:{width:1080,height:1080},deviceScaleFactor:1,locale:'en-GB',timezoneId:'UTC',colorScheme:'light',reducedMotion:'reduce'});await page.route('**/*',route=>route.abort());await page.setContent(html,{waitUntil:'load'});await page.evaluate(async()=>{await Promise.all([...document.fonts].map(f=>f.load()));await document.fonts.ready;if([...document.fonts].some(f=>f.status!=='loaded'))throw Error('Selected font did not load');await Promise.all([...document.images].map(i=>i.decode()));});
