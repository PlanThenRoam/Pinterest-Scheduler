...
async function renderInSession(session,spec,assets,loadBytes,{preflightOnly=false,scope='default'}={}){
 ...
 const browser=session.browser,page=await browser.newPage({viewport:{width:1080,height:1080},deviceScaleFactor:1,locale:'en-GB',timezoneId:'UTC',colorScheme:'light',reducedMotion:'reduce'});
 try{
  ...
  tick=clock();const png=await sharp(shot,{failOn:'warning'}).png({compressionLevel:9,adaptiveFiltering:false}).toBuffer(),meta=inspectPng(png);assert(meta.width===1080&&meta.height===1080,'Invalid output dimensions');timings.png_encoding_ms=elapsed(tick);tick=clock();
