...
function setup({invalidPdf=false,imageTimeout=false,wrongAlt=false,wrongDescription=false,proof=true}={}){
 ...
 for(const file of ['assets.ts','safety.ts','safe-edit.ts','index.ts']){const source=fs.readFileSync(base+'/'+file,'utf8').replace(/^import .*?;\s*$/gm,'').replace(/\bexport /g,'');vm.runInContext(stripTypeScriptTypes(source),c);}
