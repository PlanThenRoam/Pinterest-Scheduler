...
test('MCP discovery exposes all master actions without reading private data; calls still require auth',async()=>{
 ...
 const list=await call('tools/list'),body=await list.json();assert.equal(list.status,200);assert.equal(list.headers.get('cache-control'),'no-store');assert.equal(authChecks,0);for(const t of composer.composerTools){assert.ok(body.result.tools.find(x=>x.name===t.name));const denied=await call('tools/call',{name:t.name,arguments:{}});assert.equal(denied.status,401);}
