...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(name==="prepare_etsy_listing_update"){
   ...
   return rpc(id,output({...prepared,matched_listing:matches[0],message:"Attach the approved thumbnail as role thumbnail and all five approved listing images as listing-image-1 through listing-image-5. Update manifest.altText with six matching entries, then finalize. Do not add or change any other listing details."}));
  }
