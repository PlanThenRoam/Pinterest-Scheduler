...
const CREATIVE_CAPABILITIES = {
 version:"2.0", editor:"chatgpt-only", output:{width:1080,height:1920,aspectRatio:"9:16",audio:"silent",outerFrame:false},
 presets:CREATIVE_PRESETS, templates:ADVERT_TEMPLATES,
 global:["stylePreset","advertTemplate","advertisementIntensity","destination","season","typography","palette","photoGrade","photoMatching","textProtection","transition","transitionDuration","motionIntensity","speedCurve","overlay","atmosphere","framing","safeArea","promotion","caption","hashtags","soundRecommendation","cover"],
 scene:["purpose","text","supportingText","cta","promotionLine","duration","background","plannerPage","typography","palette","photoGrade","textLayout","textProtection","textAnimation","transition","overlay","atmosphere","framing","timeline","subjectHints","qualityOverrides"],
 ...
 plannerPage:["imageFile","sourcePdfPage","role","position","x","y","width","maxHeight","rotation","crop","cornerRadius","shadow","border","opacity","entryAnimation","exitAnimation","lockedToViewport","layerOrder","treatment","pageStack"],
 ...
 transitions:["crossfade","film-dissolve","blur-dissolve","light-sweep","dip-navy","dip-black","match-cut","soft-wipe","exposure-fade","zoom-dissolve","page-inspired","hard-cut"],
 textAnimations:["fade-in","fade-up","fade-down","soft-rise","word-fade","line-fade","tracking-reveal","masked-reveal","scale-in","editorial-slide","blur-to-sharp","static","hold-and-fade","cta-pulse"],
 patching:{format:"JSON Pointer",operations:["add","replace","remove"],example:{op:"replace",path:"/scenes/3/plannerPage/width",value:0.38}},
 rules:["approved copy is never rewritten automatically","planner pages must be genuine PDF renders","planner pages default to static viewport lock","safe areas and collision checks are mandatory","promotion expiry blocks rendering","exactly five hashtags","no fixed price unless explicitly supplied","no automatic publishing"]
};
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(name==="list_creative_styles"){
   const {data,error}=await db.from("creative_styles").select("id,name,channels,style,updated_at").eq("user_id",userData.user.id).order("name");if(error)throw error;return rpc(id,output({styles:data||[],built_in_presets:CREATIVE_PRESETS}));
  }
