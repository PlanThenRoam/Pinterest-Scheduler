...
function validate(kind:string, manifest:any){
 ...
 if(kind==="etsy"){
  if(manifest?.mode==="edit"||manifest?.updateScope==="images_only"){
   ...
   if(scopes.includes("images")){const alt=Array.isArray(manifest.altText)?manifest.altText.map((x:any)=>String(x).trim()):[];if(alt.length!==6||alt.some((x:string)=>!x))throw new Error("Image updates require exactly six matching alt-text entries.");manifest.altText=alt;manifest.imageUpdate=true;}
  ...
  if(name==="finalize_review_project"){
   ...
   if(project.kind==="etsy"){
    ...
    if(project.manifest?.mode==="edit"){
     ...
     if(scopes.includes("images")){const required=["thumbnail","listing-image-1","listing-image-2","listing-image-3","listing-image-4","listing-image-5"];if(required.some(role=>!roles.has(role)))throw new Error("Attach the replacement thumbnail and all five replacement listing images before finalizing.");}
