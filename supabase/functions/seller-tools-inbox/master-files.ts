...
export async function handleMasterTool(name:string,args:any,ctx:any){
 ...
 if(name==='create_master_file'){
  ...
  if(args.category==='blueprint'){const title=checkMetadata(args,true).title;const existing=result(await db.from('seller_master_records').select('*').eq('user_id',userId).eq('category','blueprint').eq('title',title).maybeSingle());if(existing)return {master:existing};args={...args,title};delete args.listing_id;}
