...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(body.action==='list_boards')return json({boards:await listBoards()});
  ...
  const boards=await listBoards();if(!boards.some(b=>b.id===pin.boardId))throw new Error('The selected Pinterest board is no longer available.');
  ...
  const bytes=new Uint8Array(await image.data.arrayBuffer());let binary='';for(let i=0;i<bytes.length;i+=8192)binary+=String.fromCharCode(...bytes.subarray(i,i+8192));
  ...
  const created=await pinterestRequest('/pins',{method:'POST',body:JSON.stringify({board_id:pin.boardId,title:pin.title,description:pin.description,alt_text:pin.altText||pin.title,link:pin.link,media_source:{source_type:'image_base64',content_type:image.data.type,data:btoa(binary)}})});
  ...
  const verified=await pinterestRequest('/pins/'+encodeURIComponent(created.id));
  ...
  const done=await admin.from('review_projects').update({status:'published',published_at:new Date().toISOString(),last_error:null}).eq('id',project.id);if(done.error)throw done.error;
