...
export async function handleComposer(name:string,args:any,{admin,userId}:any){
 ...
 if(name==='get_composer_catalog'){
  ...
  const r=await q;checked(r);return {assets:r.data,next_offset:offset+limit<(r.count||0)?offset+limit:null,total:r.count,...(args.include_options?{fonts:FONT_CATALOG,profiles:PROFILES,presets:PRESETS,renderer_start:{configured:dispatchConfigured(),backup:'scheduled_workflow'}}:{}),output_type:args.output_type||'square',dimensions:size,renderer_version:VERSION};
 }
 ...
 if(name==='update_marketing_composition'){
  ...
  const saved=checked(await admin.from('composer_compositions').update({spec:args.composition,layout,assets,revision:c.revision+1,status:'draft',result:null,approved_revision:null,lease:null,lease_until:null,queued_at:null,started_at:null,completed_at:null,timings:{},updated_at:new Date().toISOString()}).eq('id',c.id).eq('revision',c.revision).neq('status','cancelled').select('*').single());return response(admin,saved);
 }
