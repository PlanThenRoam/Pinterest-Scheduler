import {canonical,hash,assert,outputSize,resolveLayout} from './core.mjs';
import {resolveAutomatic,validateBrief,designSignature,LAYOUT_FAMILIES,TREATMENTS,CTA_TREATMENTS,LOCK_FIELDS,PRESET_VERSION,SIMILARITY_THRESHOLD} from './art-direction.mjs';
...
const emphasis=array(obj({start:{type:'integer',minimum:0},end:{type:'integer',minimum:1},weight:{type:'integer',minimum:100,maximum:900},colour:str(7),scale:{type:'number',minimum:1,maximum:1.65}},['start','end']),8);
const block=obj({font_family:str(80),weight:{type:'integer',minimum:100,maximum:900},italic:bool,size:{type:'number',minimum:30,maximum:180},line_height:{type:'number',minimum:1,maximum:1.5},tracking:{type:'number',minimum:-5,maximum:5},align:choice(['left','center','right']),colour:str(7),uppercase:bool,emphasis,x:{type:'number',minimum:0},y:{type:'number',minimum:0},width:{type:'number',exclusiveMinimum:0},height:{type:'number',exclusiveMinimum:0}},['font_family','weight','italic','size','line_height','tracking','align','colour']);
export const designSchema=obj({colour_mode:choice(['auto','fixed']),version:{type:'string',const:PRESET_VERSION},system_id:str(80),font_category:str(80),hierarchy:str(80),treatment:choice(TREATMENTS),layout_family:choice(LAYOUT_FAMILIES),role:choice(['hook','proof','cover_cta','pin']),fonts:array(str(80),2),blocks:obj({headline:block,supporting_copy:block,cta:block},['headline']),palette:obj({id:str(80),ink:str(7),panel:str(7),accent:str(7)},['id','ink','panel','accent']),cta:obj({treatment:choice(CTA_TREATMENTS),location:choice(['below_copy','below_headline','footer','side','offer'])},['treatment','location']),seed:str(120),background_source_id:uid},['version','system_id','font_category','hierarchy','treatment','layout_family','role','fonts','blocks','palette','cta','seed','background_source_id']);
...
export async function syncManualHistory(admin:any,userId:string,campaignId:string){
 ...
 const signatures={carousel:designSignature(rows.filter((r:any)=>r.spec.output_type==='square').map((r:any)=>r.spec)),pins:rows.filter((r:any)=>r.spec.output_type==='pinterest').map((r:any)=>designSignature([r.spec]))};
